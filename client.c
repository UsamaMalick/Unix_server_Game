...
int main(void)
{
        ...
        printf("Connected\n");
	strcpy(client_message, "Handshak!!");
	...
	if(strcmp(server_message, "Server is full.") != 0)
	{
		memset(server_message,'\0',sizeof(server_message));
		//Authentications
		...
		if(strcmp(server_message, "You are not authorized.") != 0)
		{
			int flag1 = 0;
			while(flag1 == 0)
			{
				...
				if(strcmp(server_message, "Please enter correct symbol.") == 0)
				{
					if(recv(socket_desc, server_message, sizeof(server_message),0) < 0)
					{
						printf("Receive Failed. Error!!!!!\n");
